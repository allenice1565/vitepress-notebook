...
也不是为了写声明文件才学习关于声明文件的编写，主要是为了了解ts中如何定义一个变量、函数、类、命名空间、模块的类型。平时开发中经常看到一些类型定义，但是缺乏一些基础知识，导致自己写类型定义的时候容易出错。

比如看到有些地方使用三斜线指令，有些地方使用import导入类型，有些地方在.d.ts文件中定义的类型可以全局范围使用，我自己写却不行。

学习了[流浪小猫](https://github.com/xcatliu/)大佬写的[声明文件](https://ts.xcatliu.com/basics/declaration-files.html#declare-module)后，对声明文件和类型声明有了进一步的认识。

为了避免宝藏文章消失在网络中，对[声明文件](https://ts.xcatliu.com/basics/declaration-files.html#declare-module)进行摘抄。
